var serialport = require( 'serialport' );
var SerialPort = serialport.SerialPort;
  
var serial = new SerialPort( '/dev/tty.usbserial-A9007Mxq', { 
	parser: serialport.parsers.readline( '\n' ) 
} );

serial.on( 'data', function( data ) {
	console.log( 'Echo: ' + data.toString() );
} );

var WebSocketServer = require( 'websocket' ).server;
var Http = require( 'http' );

var clients = new Array();

var server = Http.createServer( function( request, response ) {
    // Process HTTP request
	// Just Web Sockets here so nothing to see here
	// Move along!
} );
server.listen( 8080, function() {;} );

// Create Web Socket server
var sockets = new WebSocketServer( {
    httpServer: server
} );

// Web Socket server implementation
sockets.on( 'request', function( request ) {
    var connection = request.accept( null, request.origin );

	// Keep track of the clients
	clients.push( connection );

    // Handle client messages
	connection.on( 'message', function( message ) {
		var values = null;
		
        if( message.type === 'utf8' ) 
		{
			values = message.utf8Data.split( ',' );
			
			serial.write( pad( values[0] ) );			
			serial.write( pad( values[1] ) );						
			serial.write( pad( values[2] ) );			
						
			console.log( 'From client: ' + message.utf8Data );
        }
    } );

	// Close the connection
	// Probably should remove client from list
    connection.on( 'close', function( connection ) {;} );
} );

function pad( value )
{
	while( value.length < 3 )
	{
		value = '0' + value;	
	}
	
	return value;
}